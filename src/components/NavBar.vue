<template>
  <nav class="nav">
    <div class="nav__btn" :class="{ active: Active }" @click="navActive">
      <span></span>
    </div>
    <ul class="nav__list" :class="{ active: Active }">
      <li>
      ...
      <li>
        <router-link class="nav__link" to="/portfolio">
...
export default {
  data() {
    return {
      Active: JSON.parse(!localStorage.active) ? false : JSON.parse(localStorage.getItem('active'))
    }
  },
  methods: {
    navActive() {
      if (JSON.parse(localStorage.active) == true) {
